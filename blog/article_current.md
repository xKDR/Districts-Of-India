...
The official statistics that describe the Indian economy are, for the most part, produced at the level of the state, once a year, with a lag of a year or more. Policy questions do not respect this. Infrastructure, industrial concentration and urbanisation play out at the level of the district, and the district is where the Indian statistical system is weakest. There is no official measure of economic activity in a district at a monthly frequency. There is no official measure at all of the stock of physical capital in a district, or of how fast it is changing. For much of what we would like to know about a district, the source is still the Census, and the last Census was in 2011.

This matters. Lall and Chakravorty (2005) find that the spatial concentration of industry within India is itself a primary cause of income inequality across regions, and that this concentration takes place at a resolution finer than the state. Aggregated, lagged statistics hide much of the story.

We have built a new dataset that addresses this problem from outer space. For each of 641 Indian districts, it gives two measures. The first is nighttime lights, monthly, from 2014 onwards: a flow measure of economic activity. The second is building volume, annual, from 2016 to 2023: a stock measure of physical capital. Both are derived from satellite imagery, both are released as CSVs, and the code that produces them is public.
...
The difficulty is that raw satellite data is not usable as is. It arrives in unfamiliar formats, it must be corrected for a long list of biases, and processing it at the scale of a country requires serious computation. A researcher who wants a district-level series faces months of engineering before the first regression. The purpose of this dataset is to do that engineering once, in public, so that nobody has to do it again.
...
Raw VIIRS data is not usable as is. In earlier work at XKDR Forum (Patnaik, Shah and Thomas, 2022) we catalogued the problems. There are four that matter. First, bloom: bright pixels spill light into their darker neighbours. Second, background noise: pixels with no lights at all do not report zero. Third, outliers: gas flares and fires produce readings that have nothing to do with economic activity. Fourth, and most important for India, seasonal attenuation: in the monsoon months there are fewer cloud-free observations, and the radiance that is recorded is biased downwards. There is also a marked and poorly understood correlation between the number of cloud-free observations and the measured radiance (Patnaik et al., 2021). A new user of the data will not know to correct for this, and if the correction is not made, the seasonal cycle of the clouds shows up as a seasonal cycle of the economy.

Because lights have been studied for so long, these problems are well understood and the methods to correct them exist. We take the raw data and run it through the PSTT2021 pipeline of Patnaik et al. (2021), as implemented in [NighttimeLights.jl](https://github.com/xKDR/NighttimeLights.jl), which corrects the bulk of these problems, and then aggregate to the district. The map below shows the result for 2021.

![Nighttime lights by district](figures/fig15b_district_coverage_ntl.png)

*Mean radiance per pixel, 2021, for 639 of the 641 districts. Dark is dim.*
...
There is one respect in which building volume has more potential than lights, and it is the reason we are optimistic about it despite these problems. A building has a height, and the height can be measured on the ground. Municipal building registers, property tax rolls and structures of known height give a ground truth against which the satellite estimate can be checked and corrected. Seth, Singh and Uday (2026) have begun to do this in Karnataka: working with building volume inside urban local body boundaries, they find that the summed volume of a body explains much of the variation in its property tax demand. There is no comparable ground truth for the radiance of a district. We are working on a bias correction for building heights that uses known buildings as anchors for the distribution, and we expect that the cleaning methodology for this data will develop in the coming years, as it did for lights.

### Putting the two together

Each measure answers a straightforward question on its own. Together they answer a more useful one. Building volume is the stock of capital in a district; lights are the flow of activity running through it. The difference in tempo is easiest to see when the two are drawn on the same axis. Building volume gives eight annual points across the period; lights give twelve times as many.
